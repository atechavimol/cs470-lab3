...
public class theRobot extends JFrame {
    ...
    void updateProbabilities(int action, String sonars) {
        ...
        for(int i = 1; i < probs.length - 1; i++) {
            for(int j = 1; j < probs[0].length - 1; j++) {

                if(mundo.grid[i][j] == 0) {
                    double belSum = 0;
                    Map<Integer, int[]> directionCoordinates = new HashMap<>();

                    directionCoordinates.put(0, new int[]{i, j + 1});
                    directionCoordinates.put(1, new int[]{i, j - 1});
                    ...
                    for (Integer key: directionCoordinates.keySet()) {
                        ...
                        if(key == action ){
                            belSum += moveProb * probs[x][y];
                        } else {
                            belSum += ((1 - moveProb) / 4) * probs[x][y];
                        }
                    }
        ...
        for(int i = 1; i < probs.length - 1; i++) {
            for(int j = 1; j < probs[0].length - 1; j++) {
                int n_wall =  mundo.grid[i][j + 1];
                int s_wall = mundo.grid[i][j - 1]; 
                int e_wall =  mundo.grid[i - 1][j];
                int w_wall = mundo.grid[i + 1][j]; 

                int[] walls = {n_wall, s_wall, e_wall, w_wall};
                
                double sensor = 1;

                for(int k = 0; k < sonars.length(); k++) {
                    if ((int)sonars.charAt(k) == walls[k]) {
                        sensor *= sensorAccuracy;
                    } else {
                        sensor *= 1 - sensorAccuracy;
                    }
                }
   
                probs[i][j] = belPrime[i][j] * sensor;
        ...
        // }
        myMaps.updateProbs(probs); // call this
    }
    ...
    void valueIteration(){
        ...
        while(!convergenceReached) {
            double[][] newValIter = new double[mundo.width][mundo.height];
        ...
        for (int i = 0; i < mundo.width; i++) {
            for (int j = 0; j < mundo.height; j++) {
                System.out.print(String.format("%.2f", valIter[j][i]) + " ");
            }
